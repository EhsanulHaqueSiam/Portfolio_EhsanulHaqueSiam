    ...
    >
      ...
      <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden">
        ...
        >
          ...
          >
            {[
              { href: profile.github, label: 'Gh', icon: '⌘' },
              { href: profile.linkedin, label: 'Li', icon: '◈' },
            ].map((social) => (
              <MagneticHover key={social.label} strength={30}>
                <a
                  href={social.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center text-gray-500 hover:text-white hover:border-violet-500/50 transition-all duration-300 text-sm font-mono"
                >
                  {social.label}
                </a>
              </MagneticHover>
