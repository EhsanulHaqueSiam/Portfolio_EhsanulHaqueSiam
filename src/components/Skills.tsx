    ...
    <section id="skills" className="scroll-mt-28">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {skills.categories.map((category, i) => (
          <BlurFade key={category.name} delay={0.05 + i * 0.04} inView className="h-full">
            <GlowCard contentClassName="p-4" cursorEmoji={categoryEmojis[i % categoryEmojis.length]}>
              ...
              <div className="flex flex-wrap gap-2">
                {category.skills.map((skill) => (
                  <span
                    key={skill.name}
                    className="inline-flex items-center gap-1.5 rounded-sm bg-secondary px-2 py-1 text-xs font-semibold leading-none tracking-tight text-secondary-foreground transition-transform duration-200 hover:-translate-y-0.5 hover:scale-105"
                  >
